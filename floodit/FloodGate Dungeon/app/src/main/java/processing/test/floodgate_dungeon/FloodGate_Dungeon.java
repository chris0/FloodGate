...
public class FloodGate_Dungeon extends PApplet {
...
int lastColor;     // color to display on cutscene
...
int[] colors = {C1, C2, C3, C4, C5, C6};      // 1D array of 6 colors
int[][] cArray;    // 2D array of colors
...
{
  size(displayWidth, PApplet.parseInt(1.24f*displayWidth), P2D);
  w=displayWidth; h=PApplet.parseInt(1.24f*displayWidth);
  orientation(PORTRAIT);
  ow=340.0f; oh=420.0f;    // original width, height: 340x420
    ...
    if(mouseX>rw*130 && mouseX<rw*205 && mouseY>rh*185 && mouseY<rh*215) {
      ...
      for(int i = 0; i < cArray.length; i++) {
        for(int j = 0; j < cArray[i].length; j++) {
          int b = PApplet.parseInt(random(6));
          int c = colors[b];
          cArray[i][j] = c;
          sArray[i][j] = false;
        }
...
public boolean checkNeighbor(int xpos, int ypos) {
...
public void draw() {
  ...
  if(end == false)
  {
    textSize(PApplet.parseInt(rh*14));  
    if (gameState == 3) {
        background(225);
        if (tutLevel == 0) {
          fill(30);
          textAlign(CENTER);
          textSize(PApplet.parseInt(rh*35));
          text("Floodgate Dungeon",w/2,rh*30);
          textSize(PApplet.parseInt(rh*20));
          text("How To Play",w/2,rh*60);
          textSize(PApplet.parseInt(rh*15));
        ...
        if (tutLevel == 1) {
          ...
          textSize(PApplet.parseInt(rh*13));
          ...
          strokeWeight(1);
          fill(30);
          textSize(PApplet.parseInt(rh*30));
...
public void switchMode() {
  lastC2 = C2;
  doorMode = PApplet.parseInt(random(3));
  endMode = doorMode;
  C6 = color(0);
  switch(doorMode) {
  case 0:
...
public void cutsceneAnim() {
  ...
  if (cutscene <= 200) {
    ...
    if (sShake[0] == 0) {sShake[1] -= 0.9f;}
    if (sShake[1] < -11.1f && sShake[0] == 0) {sShake[0] = 1;}
    if (sShake[0] == 1) {sShake[1] += 0.9f;}
    if (sShake[1] > 11.1f && sShake[0] == 1) {sShake[0] = 0;}  
    ...
    textSize(PApplet.parseInt(rh*(14 - 13*(cutscene/200))));
    fill(30);
...
public boolean between(float pos, float min, float max)
{
...
public void explosion(float x, float y, int counter)
{
  fill(50);
  rectMode(CENTER);
...
public void reset()
{
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "FloodGate_Dungeon" };
    if (passedArgs != null) {
